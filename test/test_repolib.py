...
# Stub import of Deb822 which is used in AptRepoFile
sys.modules['debian.deb822.Deb822'] = StubDeb822
from subscription_manager.repofile import Repo, TidyWriter, YumRepoFile, AptRepoFile
...
class AptRepoFileTest(unittest.TestCase):
    ...
    def test_includes_yum_parameters(self):
        # Setup mock
        ar = AptRepoFile()
        yr = YumRepoFile()
        mock_repo = Repo('mock', [('baseurl', 'https://example.org.org/'),
        ...
        # Yum parameters must be included in Apt as well (garuanty compatibility)
...
"""
    ...
    def test_delete_repo(self):
        ...
        ar.add(mock_repo_c)
        # Modify data
        ar.delete(mock_repo_b)
        # Test modification
        self.assertEqual(len(ar.repos822), 2)
        for repo822 in ar.repos822:
            self.assertTrue('a' == repo822['id'] or 'c' == repo822['id'])

    def test_update_repo(self):
        # Setup mock
        ar = AptRepoFile()
        mock_repo_a = Repo('a', [('baseurl', 'url')])
        ar.add(mock_repo_a)
        # Modify data
    ...
    def test_fix_content_default(self):
        ...
        ar = AptRepoFile()
        repo_mock = Repo('mock', [('baseurl', exp_params['URIs']),
    ...
    def test_fix_content_arches(self):
        # Setup mock and expected values
        exp_params = {
            'arches': 'all',
            'Architectures': 'all',
    ...
    def test_fix_content_arches_multi(self):
        # Setup mock and expected values
        exp_params = {
            'arches': 'amd64 i386',
