...
class SupportedArchesCollector(collector.FactsCollector):
    ...
    def get_arches_on_debuntu(self):
        ...
        try:
            arch = subprocess.check_output("dpkg --print-architecture", shell=True).decode('UTF-8')
            if arch != '':
        ...
        try:
            arch = subprocess.check_output("dpkg --print-foreign-architectures", shell=True).decode('UTF-8')
            if arch != '':
                arches.append(arch.rstrip('\n'))
        except Exception as e:
    ...
    def get_all(self):
        ...
        :return: dictionary containing architectures
        """
        dist_name = self._collected_hw_info['distribution.name']
